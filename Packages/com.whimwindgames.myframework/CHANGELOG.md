...
## [1.1.0-preview.47] - 2026-09-11

### Added

- `Frame_Base` 新增可共用的 `FrameSafeArea` 组件，复用 `FrameScreenContext`，对已有内容容器进行安全区域内缩，可选最小逻辑尺寸与等比缩放，支持启动阶段和热更新页面。
- 保留 Prefab 原有层级、绑定和控件布局，背景可继续全屏；支持动态窗口/方向/安全区变化、无效样本保护及禁用恢复。
- 新增安全区域接入说明与 EditMode 回归测试。

## [1.1.0-preview.46] - 2026-09-02

### Added

- `GameEntryBase` 新增框架更新前的显示区域同步入口，World Space UI 项目可在同一帧完成自定义逻辑视口调整。
- 屏幕快照变化时，AOT 与热更新布局管理器会刷新所有已创建页面的根锚点，支持运行中调整窗口大小和切换全屏。

### Fixed

- `ScaleAnchor`、`ScaleAnchor3D` 与 `ResScaleAnchor` 在强制刷新时会重新读取当前UI根尺寸，同时保留初始设计数据，避免动态缩放累计误差。
- 屏幕方向系统持续同步窗口像素尺寸，避免窗口变化后输入坐标仍使用启动时分辨率。
...
- 内容寻址客户端在一次完整更新期间持续持有跨 Base 的可重入进程锁，避免 macOS 上不同 Base 副本并发规划、迁移或读取时与共享 Blob GC 交错。
