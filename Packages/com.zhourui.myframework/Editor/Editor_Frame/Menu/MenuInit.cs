...
public class MenuInit
{
    ...
    private const string HYBRIDCLR_PACKAGE_URL = "https://github.com/focus-creative-games/hybridclr_unity.git#4feac30cb2e105992986c737f7f54992b8300e1a";	// HybridCLR官方固定版本
    private const string HYBRIDCLR_PACKAGE_FALLBACK_URL = "https://gitee.com/focus-creative-games/hybridclr_unity.git#4feac30cb2e105992986c737f7f54992b8300e1a";	// HybridCLR镜像
    private const string OBFUZ_PACKAGE_URL = "https://github.com/focus-creative-games/obfuz.git#7b06f0fed8675c31cbae9760a93e7deec0c0a05c";					// Obfuz官方固定版本
    private const string OBFUZ_PACKAGE_FALLBACK_URL = "https://gitee.com/focus-creative-games/obfuz.git#7b06f0fed8675c31cbae9760a93e7deec0c0a05c";					// Obfuz镜像
    ...
    private static string mPackageFallbackUrl;																		// 当前Package备用安装地址
    private static bool mPackageFallbackTried;																		// 是否已经尝试备用地址
    ...
    protected static void startInstallPackage(string packageUrl)
    {
        ...
        mPackageFallbackUrl = getPackageFallbackUrl(packageUrl);
        mPackageFallbackTried = false;
        startPackageAddRequest(packageUrl);
    }
    // 创建Package安装请求
    private static void startPackageAddRequest(string packageUrl)
    {
    ...
    // 获取与官方地址指向同一提交的备用镜像
    private static string getPackageFallbackUrl(string packageUrl)
    {
        if (packageUrl == HYBRIDCLR_PACKAGE_URL)
        {
            return HYBRIDCLR_PACKAGE_FALLBACK_URL;
        }
        if (packageUrl == OBFUZ_PACKAGE_URL)
        {
            return OBFUZ_PACKAGE_FALLBACK_URL;
        }
        return null;
    }
    // 等待Package安装完成
    protected static void updateInstallPackage()
    {
        ...
        string errorMessage = mPackageAddRequest.Error.message;
        mPackageAddRequest = null;
        EditorApplication.update -= updateInstallPackage;
        if (!mPackageFallbackTried && !string.IsNullOrEmpty(mPackageFallbackUrl))
        {
            string fallbackUrl = mPackageFallbackUrl;
            mPackageFallbackTried = true;
            Debug.LogWarning("Package官方地址安装失败,尝试备用镜像: " + errorMessage);
            startPackageAddRequest(fallbackUrl);
            return;
        }
        mPackageFallbackUrl = null;
        mPackageFallbackTried = false;
        Debug.LogError("Package安装失败: " + errorMessage);
        SessionState.SetInt(SESSION_HYBRIDCLR_OBFUZ_STATE, (int)HYBRIDCLR_OBFUZ_STATE.NONE);
    }
